...
public class EntityModifier {
     ...
     */
    public static void ensureKindExists(EntityDelta state, PlacesSource source, String mimeType) {
    ...
    public static ValuesDelta insertChild(EntityDelta state, DataKind kind, EditType type) {
        ...
        // Our parent Place_ID is provided later
        ...
        //Log.d(TAG, "insertChild: child=" + child.toString());
     ...
     */
    public static void trimEmpty(EntityDelta state, PlacesSource source) {
        ...
        for (DataKind kind : source.getSortedDataKinds()) {
            ...
            for (ValuesDelta entry : entries) {
                ...
                final boolean isGoogleSource = false;
     ...
     */
    public static void parseExtras(Context context, PlacesSource source, EntityDelta state,
            Bundle extras) {
        ...
        {
        	final boolean hasWifi = extras.containsKey(Insert.WIFI_FINTERPRINT);
        	if (hasWifi) {
        		//Log.d(TAG, "parseExtras: has Wifi.");
        		String fingerprint = extras.getString(Insert.WIFI_FINTERPRINT);
        		String timestamp = extras.getString(Insert.TIME);
        		//Log.d(TAG, "parseExtras : time=" + timestamp);
        		final ValuesDelta entry = EntityModifier.insertChild(state, source.getKindForMimetype(WifiFingerprint.CONTENT_ITEM_TYPE));
     				entry.put(WifiFingerprint.FINGERPRINT, fingerprint);
     				entry.put(WifiFingerprint.TIMESTAMP, timestamp);
     				//Log.d(TAG, "after insert : " + entry.toString());
